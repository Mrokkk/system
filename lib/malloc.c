...
[[noreturn]] static void die(const char* fmt, ...)
{
    ...
    [[noreturn]] extern void LIBC(abort)();
    LIBC(abort)();
}
...
#define CLASS_METADATA_SET(metadata, size) \
    class_metadata_set( \
...
#define CLASS(size, slab_size) \
    CLASS_METADATA_SET(metadata, size);
...
static void* allocate_small(size_t size)
{
    ...
    slab_allocator_t* allocator = slab_allocator_find(size);
    ...
    void* brk_ptr = sbrk(allocator->mem_size);
    ...
    slab = metadata_alloc(sizeof(*slab));
...
found_region:
    ...
    return PTR(slab->start + slot * allocator->class_size);
}
...
void LIBC(free)(void* ptr)
{
    ...
    {
        if (UNLIKELY(munmap(PTR(large_region->start - PAGE_SIZE), large_region->end - large_region->start + 2 * PAGE_SIZE)))
        {
            DIE("munmap failed: %s", strerror(errno));
        }
        list_del(&large_region->list_entry);
        list_add(&large_region->list_entry, &metadata->free.large_regions);
        return;
    }
...
found:
    offset = ADDR(ptr) - slab->start;

    if (UNLIKELY(offset % allocator->class_size))
    {
        DIE("%x not aligned for class size %u", ptr, allocator->class_size);
    }
...
static void* small_realloc(void* ptr, size_t size, slab_allocator_t* allocator, slab_t* slab)
{
    ...
    {
        DIE("not aligned ptr %x for class size %u", ptr, class_size);
    }
