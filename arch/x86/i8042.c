...
static uint8_t i8042_status_read(void)
{
...
int i8042_send_cmd(uint8_t byte)
{
    ...
    if (unlikely(errno = i8042_wait_write()))
    {
        return errno;
    }
...
static uint8_t i8042_device_reset(uint8_t port)
{
    uint8_t data;
...
static uint16_t i8042_device_detect(uint8_t port)
{
    ...
    if (i8042_status_read() & I8042_OUTPUT_BUFFER)
    {
        res |= i8042_receive() << 8;
    }
...
finish:
    ...
    log_notice("[port0] device %04x (%s)", port0_device, i8042_device_string(devices[I8042_KBD_PORT]));
    log_notice("[port1] device %04x (%s)", port1_device, i8042_device_string(devices[I8042_AUX_PORT]));
